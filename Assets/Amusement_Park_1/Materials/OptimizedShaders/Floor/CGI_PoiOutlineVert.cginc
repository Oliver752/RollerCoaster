...
v2f vert(appdata v)
{
	...
	{
		distanceOffset *= min(distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, localPos).xyz), (0.0 /*_OutlinesMaxDistance*/));
	}
	float3 offset = o.normal * ((5.0 /*_LineWidth*/) * (1.0 /*_commentIfZero_EnableOutlinePass*/) / 100) * outlineMask * distanceOffset;
	...
	{
		offset = mul((float3x3)transpose(UNITY_MATRIX_V), float4(0.54,0,0,0));
		offset *= distanceOffset;
	}
